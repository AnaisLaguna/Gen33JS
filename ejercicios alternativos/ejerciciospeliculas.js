const movies = [
   ...
   // Ejercicios:Crear funciones que cumplan con los siguientes requerimientos:
    ...
    console.log(directors);
     ...
     const nombreActores = movies.reduce((acc, movie) => {
        movie.protagonists.forEach((actor) => {
          if (!acc.includes(actor.name)) {
            acc.push(actor.name);
          }
        });
        return acc;
     ...
     const ratings = movies.reduce((acc, movie) => {
      ...
      console.log(ratings);
     //5. Obtener una lista de las películas que tienen una duración entre 2 rangos, 
     //por ejemplo las películas que duran entre 100 y 120 mins ( los valores de las duraciones deben ser dinámicos)
            const minDuracion = 100;
            const maxDuracion = 120;

            const filtroPeliculas = movies.filter(movie => {
            const duracionPelicula = Number(movie.duration.replace(' minutos', ''));
            return duracionPelicula >= minDuracion && duracionPelicula <= maxDuracion;
            }).map(movie => ({
            title: movie.title,
            ...
            function buscarpelisRating(rating) {
                return movies.filter(movie => movie.rating === rating);
              }
            
              const ratingPeliculas = buscarpelisRating("R");
              console.log("\nRespuesta ejercicio 6: ")
              console.log(ratingPeliculas);
              
             
     /*7. Obtener una lista de películas películas estrenadas en un rango de años 
     (por ejemplo, entre 2000 - 2010, los valores de los años deben ser dinámicos)*/
     
            function rangoPeliculas(inicioAños, finAños) {
                return movies.filter(movie => movie.releaseYear >= inicioAños && movie.releaseYear <= finAños);
            }
            
            
            const peliculasFiltradas = rangoPeliculas(2000, 2010);
     ...
     //10. Obtener la cantidad de películas de cada clasficación. Esta información debe estar organizada de la siguiente forma:
        /*  {
           ...
           //    Es decir, la propiedad del objeto resultante deber ser la clasificación, y el valor de esa propiedad debe ser la cantidad d
     //e películas que pertenecen a esa clasificación
          function clasificacionPelis() {
          ...
          const movieRatingCount = clasificacionPelis();
      ...
      /*11. Obtener la cantidad de películas de cada país, organizada de la siguiente forma:
          ...
          function clasificacionPelispais() {
            return movies.reduce((acc, movie) => {
          ...
          const repeticionProtagonista = (movies, actor) => {
            ...
            return `Este actor ${actor} aparece en ${lista} peliculas de la lista`;
          };
          
          console.log("\nRespuesta ejercicio 13: ")
          console.log(repeticionProtagonista(movies, "Kevin Spacey"));
        ...
        const formatoPelis = (movies) => {
            let list = movies.map((movie) => {
              let reestructuracion = movie.synopsis.split(" ").slice(0, 10);
          
              return `Title: "${movie.title} - ${movie.director} - ${
                movie.duration
              }", Synopsis: "${reestructuracion.join(" ")}..."`;
            });
            return list;
          };
          
          console.log("\nRespuesta 14:");
          console.log(formatoPelis(movies));
